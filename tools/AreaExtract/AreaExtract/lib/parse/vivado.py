...
def to_vivado_rsrc(d: dict) -> VivadoRsrc:
    mapping = {
        "Total LUTs": "lut",
        "Logic LUTs": "llut",
        "LUTRAMs": "lutram",
    ...
    kwargs = {}
    for rpt_key, field_name in mapping.items():
        raw_value = d.get(rpt_key, "0").strip()
        kwargs[field_name] = int(raw_value) if raw_value.isdigit() else 0

    return VivadoRsrc(**kwargs)


def to_cell(d: dict) -> Cell:
    return Cell(
        name=d["Instance"],
        type=d["Module"],
        generated=False,
        rsrc=to_vivado_rsrc(d),
    )


def to_design(flat_tree: dict) -> Design:
    return {fq_name: to_cell(node) for fq_name, node in flat_tree.items()}


def rpt_to_design_with_metadata(filename: str) -> DesignWithMetadata:
    tree = create_tree(filename)
    flat_tree = flatten_tree(tree)
    design = to_design(flat_tree)
    metadata = Metadata(origin="Vivado")
    return DesignWithMetadata(design=design, metadata=metadata)
