# Metaprogramming script for generating FIRRTL definitions of primitives
# used in a Calyx program.
# Inputs:
# FIRRTL_FILE - core program in FIRRTL
# (generated by the FIRRTL backend)
# PRIMITIVE_USES_JSON - JSON denoting primitive uses in the Calyx program
# (generated by primitive-uses backend)
# Outputs to stdout a whole FIRRTL program
# (combining the core program and necessary primitives)
...
def retrieve_firrtl_template(primitive_name):
    ...
    if not (os.path.isfile(firrtl_file_path)):
        print(
            f"{sys.argv[0]}: FIRRTL template file for primitive \
                {primitive_name} does not exist! Exiting..."
        )
        sys.exit(1)
    return firrtl_file_path


# Generates a primitive definition from the provided JSON
# data of a unique primitive use
