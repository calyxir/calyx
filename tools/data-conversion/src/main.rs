...
fn main() {
    ...
    convert(&args).expect("Conversion failed");
}
...
fn convert(args: &Arguments) -> io::Result<()> {
    // Extract arguments
    ...
    let output_format = &args.output_format; // New flag: "text", "binary", or "stdout"
    ...
    match output_format.as_str() {
        ...
        "stdout" => {
            // Write to standard output using the `to_std_out` function
            to_std_out(filepath_get)?;
        }
        _ => {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "Invalid output format. Use 'text', 'binary', or 'stdout'.",
            ));
        }
    }
