...
fn testbench_error_unknown_tb<S: AsRef<str>>(tb: S) -> TestbenchResult {
    TestbenchResult::Err(std::io::Error::other(format!("Unknown testbench '{}'", tb.as_ref())))
}
...
pub trait Testbench {
    ...
    fn run(&self, input: String, tests: &[String], work_dir: TempDir) -> TestbenchResult;
}
...
impl TestbenchManager {
    ...
    pub fn run<S: AsRef<str>>(&self, name: S, input: String, tests: &[String]) -> TestbenchResult {
...
fn copy_into<S: AsRef<str>>(file: S, work_dir: &TempDir) -> std::io::Result<String> {
