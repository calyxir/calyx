...
for dir_name in os.listdir('plugins'):
    ...
    if os.path.isdir(dir_path):
        subprocess.run(['cargo', 'build', '--manifest-path', os.path.join(dir_path, 'Cargo.toml'), '--release', '--target-dir', os.path.join(dir_path, 'target')], check=True)
        ...
        for file_name in os.listdir(release_dir):
            ...
            if os.path.isfile(file_path) and (file_name.endswith('.dylib') or file_name.endswith('.so')):
                os.remove(os.path.join('plugins', file_name))
