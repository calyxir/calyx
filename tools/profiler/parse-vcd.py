...
def remap_tdcc_json(tdcc_json_file, groups_json_file, components_to_cells):
    profiling_infos = json.load(open(tdcc_json_file))
    ...
    # add all groups we missed
    all_groups = json.load(open(groups_json_file))
    for component_info in all_groups:
        component = component_info["component"]
        for cell in components_to_cells[component]:
            for group in component_info["groups"]:
                group_names[f"{cell}.{group}"] = component
...
if __name__ == "__main__":
    if len(sys.argv) > 5:
        ...
        groups_json = sys.argv[4]
        out_csv = sys.argv[5]
        dump_out_json = sys.argv[6]
        main(vcd_filename, fsm_json, cells_json, groups_json, out_csv, dump_out_json)
    else:
        args_desc = [
            "VCD_FILE",
            "TDCC_JSON",
        ...
        print("GROUPS_JSON: Run the `component_groups` tool")
        print("If SUMMARY_OUT_CSV is STDOUT, then summary CSV will be printed to stdout")
        print("DUMP_OUT_JSON: output json file for group-specific")
        sys.exit(-1)
