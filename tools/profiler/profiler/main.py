...
def create_visuals(
    ...
    if utilization_variable:
        plot_utilization_per_cycle(tracedata.trace, utilization_variable, out_dir)
        plot_cycles_per_primitive(tracedata.trace, utilization_variable, out_dir)
        plot_heatmap(tracedata.trace, utilization_variable, out_dir)
        plot_ratio(tracedata.trace, utilization_variable, out_dir)
        print(
            sorted(
                {
                    prim
                    for trace in tracedata.trace.values()
                    for prim in trace.primitives_active
                }
            )
        )


def plot_utilization_per_cycle(
    data: dict[int, UtilizationCycleTrace], var: str, out_dir: str
):
    records = [
        {"cycle": cycle_id, "value": obj.utilization.get(var, 0)}
        for cycle_id, obj in data.items()
    ]
    df = pd.DataFrame(records)
    fig = px.bar(df, x="cycle", y="value", title=f"Utilization of '{var}' over cycles")
    fig.write_html(out_dir + "/utilization-per-cycle.html")


def plot_cycles_per_primitive(
    data: dict[int, UtilizationCycleTrace], var: str, out_dir: str
):
    counter = Counter()
    ...
    fig = px.bar(
        ...
        x="primitive",
        y="cycles_used",
        title=f"Primitives with nonzero '{var}' across cycles",
    )
    fig.write_html(out_dir + "/cycles-per-primitive.html")


def plot_heatmap(data: dict[int, UtilizationCycleTrace], var: str, out_dir: str):
    aggregated = []
    usage_sum = Counter()
    active_cycles = Counter()

    for cycle, trace in data.items():
...
def plot_ratio(data: dict[int, UtilizationCycleTrace], var: str, out_dir: str):
    usage_sum = Counter()
    active_cycles = Counter()
