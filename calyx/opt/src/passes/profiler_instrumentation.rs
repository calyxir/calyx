...
fn create_assignments(
    ...
    group_names: &Vec<Id>,
    structural_enable_map: HashMap<Id, Vec<(Id, ir::Guard<Nothing>)>>,
    cell_invoke_map: HashMap<Id, Vec<Id>>,
    primitive_invoke_map: HashMap<Id, Vec<(Id, ir::Guard<Nothing>)>>,
) -> Vec<(
    Id,
    ...
    for (invoked_group_name, parent_groups) in structural_enable_map.iter() {
    ...
    for (invoker_group, invoked_cells) in cell_invoke_map.iter() {
        for invoked_cell in invoked_cells {
            let probe_cell_name = format!(
                "{}{}{}{}{}_cell_probe",
                invoked_cell, delimiter, invoker_group, delimiter, comp_name
            );
            let probe_cell =
                builder.add_primitive(probe_cell_name, "std_wire", &[1]);
            probe_cell.borrow_mut().add_attribute(BoolAttr::Control, 1);
    ...
    for (group, primitive_invs) in primitive_invoke_map.iter() {
        for (primitive_cell_name, guard) in primitive_invs.iter() {
            ...
            group_name_assign_and_cell.push((*group, probe_asgn, probe_cell));
        }
    }
