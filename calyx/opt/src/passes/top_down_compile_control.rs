...
struct Schedule<'b, 'a: 'b> {
    ...
    /// The topmost control node's position ids. This is to identify the control node that
    /// the schedule (and the control group generated by the schedule) corresponds to.
    pub topmost_node_pos: Vec<u32>,
}
...
impl Schedule<'_, '_> {
    ...
    fn realize_schedule(
        ...
        fsm_groups.insert(ProfilingInfo::Fsm(FSMInfo {
            ...
            states: states.into_iter().sorted().collect(),
            pos: self.topmost_node_pos.into_iter().sorted().collect(),
        }));
...
impl Schedule<'_, '_> {
    ...
    fn calculate_states(
        ...
        // Collect position ids from the topmost node in the schedule
        // (assumes that either all or no control nodes contain position ids,
        //  and that the topmost node is either a seq, while, or if.)
        if self.topmost_node_pos.is_empty() {
            let attrs_opt = match con {
                ir::Control::Seq(_)
                | ir::Control::While(_)
