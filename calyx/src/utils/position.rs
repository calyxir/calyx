...
use std::{cmp, fmt::Write, mem, rc::Rc, sync};
...
impl GlobalPositionTable {
    ...
    fn as_mut() -> &'static mut PositionTable {
...
impl GPosIdx {
    ...
    fn get_lines(&self) -> (Vec<&str>, usize, usize) {
        ...
        let file = &table.get_file(pos_d.file).source;
    ...
    pub fn format(&self, err_msg: &str) -> String {
        let table = GlobalPositionTable::as_ref();
        let pos_d = table.get_pos(self.0);
        let name = &table.get_file(pos_d.file).name;
