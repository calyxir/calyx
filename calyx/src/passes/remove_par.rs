...
fn resolve_conflicts(
    ...
    ctx: &Context,
        ...
        let mut add_structure =
            |st: &mut StructureGraph, comp1: &ast::Id, port1: &ast::Id, comp2: &ast::Id, port2: &ast::Id, width: u64| {
                let name = format!("{}_{}_id", comp1.to_string(), comp2.to_string());
                //TODO: check if id already exists
                let id_comp =
                    ctx.instantiate_primitive(&name, &"std_id".into(), &[width])?;
                let id = st.add_primitive(
                    &name.into(),
                    "std_id",
                    &id_comp,
                    &[width],
        ...
        for idx2 in comps2
            ...
            .into_iter(){
                for port2 in st_origin.graph[idx2].out_ports(){
                    for (node_data, edge_data) in st_origin.connected_to(idx2, port2.to_string()){
                        ...
                        if comps1.contains(comp_dest){
                            ...
                            add_structure(structure, comp_src, &port_src, comp_dest, &port_dest, edge_data.width)?;
                            modified=true;
                        }
                    }                
                }
                for port2 in st_origin.graph[idx2].in_ports(){
                    for (node_data, edge_data) in st_origin.connected_from(idx2, port2.to_string()){
