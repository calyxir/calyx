...
fn infer_latency<'a>(
    ...
    for asgn in &group.assignments {
        let src_parent_name: String =
            asgn.src.borrow().get_parent_name().to_string();

        let dst_parent_name: String =
            asgn.dst.borrow().get_parent_name().to_string();
        ...
        match (&asgn.dst.borrow().parent, &asgn.src.borrow().parent) {
            (
                ...
                if let (
                    ...
                    let data_dst =
                        latency_data.get(dst_cell_prim_type.as_ref());
                    let data_src =
                        latency_data.get(src_cell_prim_type.as_ref());
                    if let (Some((go_dst, _, _)), Some((_, done_src, _))) =
                        (data_dst, data_src)
                    {
                        if asgn.dst.borrow().name == *go_dst
                            && asgn.src.borrow().name == *done_src
                        {
                            writes.insert(
                                src_parent_name.to_string(),
                                dst_parent_name.to_string(),
                            );
                        }
    ...
    cell2latency.insert(START_NODE.to_string(), 0);
    let mut curr_node = START_NODE.to_string();
    while curr_node != group.name.to_string() {
        ...
        if w != group.name.to_string() {
            if let ir::CellType::Primitive { name, .. } =
                &comp.find_cell(&w).unwrap().borrow().prototype
            {
                let data = latency_data.get(name.as_ref());
                if let Some((_go, _done, latency)) = data {
                    cell2latency.insert(
                        writes.get(&curr_node).unwrap().to_string(),
                        *latency + cell2latency.get(&curr_node).unwrap(),
                    );
                }
            }
        } else {
            ...
            cell2latency
                .insert(w, *cell2latency.get(&curr_node.to_string()).unwrap());
        }
...
impl Visitor<()> for InferStaticTiming<'_> {
    fn start(
        ...
        for group in &comp.groups {
            //let graph = GraphAnalysis::from(&*group.borrow());
