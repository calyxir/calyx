...
fn is_data_port(pr: &RRC<ir::Port>) -> bool {
    let port = pr.borrow();
    if !port.attributes.has(ir::Attribute::Data) {
        return false;
    }
    if let ir::PortParent::Cell(cwr) = &port.parent {
        let cr = cwr.upgrade();
        let cell = cr.borrow();
        if cell.attributes.has(ir::Attribute::Data) {
            return true;
        }
    }
    false
...
fn memory_read_write(comp: &ir::Component) -> Vec<v::Stmt> {
    ...
    let memories = comp
        ...
        .iter()
        .filter_map(|cell| {
            let is_external = cell.borrow().get_attribute(ir::Attribute::External).is_some();
            if is_external
                && cell
                    .borrow()
                    .type_name()
