...
def do_fuzz_file(args):
    ...
    try:
        ...
        if prog_file_type != ".futil":
            ...
            os.system(f"fud e {original_prog} --to calyx > {args.input_file_1}")

        elif spec_file_type != ".futil":
            args.input_file_2 = spec_name + ".futil"
            os.system(f"fud e {original_spec} --to calyx > {args.input_file_2}")
        ...
        logging.error(e)
        logging.error(
            f"fail unequal output for prog: {args.input_file_1}, spec: {args.input_file_2}"
        )
...
def do_fuzz_backend(args):
    ...
    try:
        ...
        if prog_file_type != ".futil":
            ...
            os.system(f"fud e {original_prog} --to calyx > {args.input_file}")
...
def conf_check_file(parser):
    parser.add_argument(
        "-input_1", dest="input_file_1", help="Path to the input file 1", nargs="?"
    )
    parser.add_argument(
        "-input_2", dest="input_file_2", help="Path to the input file 2", nargs="?"
    )

    parser.add_argument(
        "-backend",
        dest="backend_tool",
        help="Receive the backend tool; use icarus0-verilog as " "default",
    )
...
def conf_check_backend(parser):
    parser.add_argument(
        "-input", dest="input_file", help="Path to the input file", nargs="?"
    )

    parser.add_argument(
        "-backend_1", dest="backend_1", help="Receive the first backend tool"
    )
    parser.add_argument(
        "-backend_2", dest="backend_2", help="Receive the second backend tool"
    )
...
if __name__ == "__main__":
    ...
    conf_check_backend(
        sp.add_parser("backend", help="Run fuzz to compare two backend tools.")
    )
