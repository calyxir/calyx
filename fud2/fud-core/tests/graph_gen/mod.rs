...
impl PlannerTest {
    ...
    ///
    /// This checks the plan itself is well-formed, meaning each op which is run has the inputs it
    /// required generated by a previously run op.
    ///
    /// It does not check that the plan generates all of the output states specified in a request,
    /// nor does it check that a planner finds a plan if one exists.
    pub fn eval(
        &self,
