...
impl EnumeratePathFinder {
    ...
    /// `Steps` and return a sequence such that applying `good` to the sequence results in `true.
        ...
        F: Fn(&[Step]) -> bool,
    {
        ...
        for op_ref in ops.keys() {
            ...
            let all_generated = ops[op_ref].input.iter().all(|input| {
                ...
                // `plan` is reversed so the latest versions of inputs are used.
            ...
            // Reucrse! Now that `len` has been reduced by one, see if this new problem has a
            // solution.
