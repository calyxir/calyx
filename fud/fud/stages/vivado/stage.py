...
from .extract import hls_extract, place_and_route_extract, hls_extract_cycles
...
class VivadoHLSExtractCyclesSttage(Stage):
    name = "hls-files"

    def __init__(self):
        super().__init__(
            src_state="hls-files",
            target_state="hls-cycles-estimate",
            input_type=SourceType.Directory,
            output_type=SourceType.String,
            description="Extracts latency information from Vivado HLS synthesis files",
        )
    ...
    def _define_steps(self, input, builder, config):
        ...
        def extract(directory: SourceType.Directory) -> SourceType.String:
            ...
            """
            top = config.get(["stages", self.name, "top"]) or "kernel"
            return hls_extract_cycles(Path(directory.name), top)
