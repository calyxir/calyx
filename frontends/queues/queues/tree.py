...
def insert_tree(prog, name, root, children, flow_infer):
    ...
    children = [comp.cell(f"child_{i}", child) for i, child in enumerate(children)]
    ...
    def invoke_child(child, cmd, value, ans, err):
        return cb.invoke(child, in_cmd=cmd, in_value=value, ref_ans=ans, ref_err=err)

    recurse = {
        n: invoke_child(children[n], cmd, value, ans, err) for n in range(len(children))
    }

    push_logic = [
        infer_flow,
        cb.invoke(root, in_cmd=cmd, in_value=flow.out, ref_ans=ans, ref_err=err),
        cb.if_with(err_eq_0, comp.case(flow.out, recurse)),
    ]

    pop_logic = [
        cb.invoke(root, in_cmd=cmd, in_value=flow.out, ref_ans=ans, ref_err=err),
        cb.if_with(err_eq_0, comp.case(ans.out, recurse)),
    ]
