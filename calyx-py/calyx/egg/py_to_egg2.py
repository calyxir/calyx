...
import egg_ast2 as egg
from egg_ruleset2 import calyx_ruleset
from egglog import *
...
class CalyxEgg():
    ...
    def ConvertGroup(self, group: ast.Group, component: ast.Component):
        ...
        return egg.Group(
            name=group.id.name,
            cells=Vec[String](*[String(s) for s in set.intersection(incumbent, cells)])
        )
    ...
    def ConvertControl(self, control: ast.Control, component: ast.Component, groups: dict[str, ast.Group]) -> egg.Control:
        match control:
            case ast.Enable(name, promotable):
                ...
                promotable = promotable or group.static_delay or egg.OptionalNum.none()
                return egg.Enable(self.ConvertGroup(group, component), promotable)
