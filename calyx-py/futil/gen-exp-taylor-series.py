...
FixedPoint("-1.5", 8, 4, True).pretty_print()
FixedPoint("1.5", 8, 4, True).pretty_print()
assert 0
...
def generate_cells(degree: int, width: int, int_width: int) -> List[Cell]:
    ...
    for i in range(2, degree + 1):
        ...
        reciprocal_factorials.append(
            Cell(CompVar(f"reciprocal_factorial{i}"), stdlib.constant(value, width))
        )
    # Constant values for the exponent to the fixed point `pow` function.
    constants = [Cell(CompVar(f"c{i}"), stdlib.constant(i, width))
                 for i in range(2, degree + 1)]
    one = [
        Cell(
            CompVar(f"one"),
...
def divide_and_conquer_sums(degree: int) -> List[Structure]:
    ...
    For example, with N == 4, we will produce groups:
      group sum_round1_1 { ... }     #    p1 p2  p3  p4
                                     #     \  /   \  /
      group sum_round1_2 { ... }     #    sum1   sum2
                                     #       \   /
      group sum_round2_1 { ... }     #        sum1

      # Final sum with `1`, the 0th degree in the Taylor Series.
    ...
    while sum_count > 1:
        ...
        for i, (lhs, rhs) in enumerate(register_indices):
            ...
            connections = [
                Connect(CompPort(reg_lhs, "out"), CompPort(adder, "left")),
    ...
    return groups + [Group(group_name, connections, 1), out]
...
def generate_groups(degree: int, width: int, int_width: int) -> List[Structure]:
    ...
    def multiply_by_reciprocal_factorial(i: int) -> Group:
        ...
        group_name = CompVar(f"mult_by_factorial_reciprocal{i}")
...
def generate_control(degree: int) -> Control:
    assigns = [ParComp([Enable(f"assign_x{i}") for i in range(2, degree + 1)])]
    pow_invokes = [
        ParComp(
            [
                Invoke(
                    CompVar(f"pow{i}"), [
                        ...
                        ("exp", CompPort(CompVar(f"c{i}"), "out"))
                    ], []
                )
                for i in range(2, degree + 1)
            ]
...
def generate_exp_taylor_series_approximation(
    ...
    return Program(
        imports=[Import("primitives/std.lib"), Import("primitives/bitnum/math.futil")],
        components=[
            Component(
                "exp",
...
if __name__ == "__main__":
    generate_exp_taylor_series_approximation(degree=16, width=32, int_width=16).emit()
