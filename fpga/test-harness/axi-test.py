...
async def run_vadd_test(toplevel):
    ...
    expect_path = Path("../../examples/dahlia/vectorized-add.expect")
    ...
    for mem in mems:
        data_width = vadd_tb.data_width(mem, data)
        mem_length = vadd_tb.mem_size(mem, data)
        post_execution = rams[mem].read(addr, mem_length)
        post_execution = decode(post_execution, data_width)
        print(f"mem_out is {post_execution}")
...
async def manual_vadd(toplevel):
    ...
    ram = AxiLiteRam(
        AxiLiteBus.from_prefix(toplevel, "m2_axi"), toplevel.ap_clk, size=mem_size
    )
    ...
    async def write_to_bram(data: list[int], mem):
        ...
        while mem.copy_done.value != 1 and mem.read_txn_count.value != 8:
            ...
            data_width = 32
            mem.RDATA.value = data[mem.read_txn_count.value] * 2 ** (
                mem.read_txn_count.value * data_width
            )
            mem.RVALID.value = 1
