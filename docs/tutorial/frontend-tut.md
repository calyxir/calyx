...
```
Now, running `fud check` should report that the `mrxl` and `mrxl-data` binaries are correctly installed.
**TODO(rachit).** This does not install `mrxl-data`. Where is it?
...
At a high level, we can change the compilation of the `map` operation to produce two copies of the hardware we generated above.
