...
Counterintuitively, `c`, `v`, `calyx_base`, `calyx_exe`, and `args` do not contain the actual variable values. They contain identifiers which are replaced by the values at runtime. This thwarts the possibility of the different config values or different input file names causing different code paths of a given op to run.
