...
pub struct TopDownCompileControl {
    ...
    /// Output a JSON representation of all of the component cells if specified (AYAKA FIXME: maybe take this out of TDCC?)
    dump_component_cell_json: Option<OutputFile>,
    ...
    /// Bookkeeping for components to all of their component (non-primitive) cell names --> names of original components
    component_cells: HashSet<ComponentInfo>,
...
impl ConstructVisitor for TopDownCompileControl {
        ...
        Self: Sized + Named,
    {
        ...
        Ok(TopDownCompileControl {
            ...
            dump_component_cell_json: opts[&"dump-component-cell-json"]
                .not_null_outstream(),
            early_transitions: opts[&"early-transitions"].bool(),
            fsm_groups: HashSet::new(),
            component_cells: HashSet::new(),
...
impl Visitor for TopDownCompileControl {
    fn start(
        ...
        self.component_cells.insert(curr_comp_info);
    ...
    fn finish_context(&mut self, _ctx: &mut calyx_ir::Context) -> VisResult {
        ...
        if let Some(json_out_file) = &self.dump_component_cell_json {
            let _ = serde_json::to_writer_pretty(
                json_out_file.get_write(),
