...
impl ConstructVisitor for StaticInference {
    fn from(ctx: &ir::Context) -> CalyxResult<Self> {
        ...
        for prim in ctx.lib.signatures() {
            ...
            if prim_go_done.ports.len() == 1 {
                static_component_latencies.insert(
                    prim.name,
                    NonZeroU64::new(prim_go_done.ports[0].2).unwrap(),
                );
            }
        ...
        Ok(StaticInference {
            latency_data,
            static_component_latencies: static_component_latencies,
        })
    }
...
impl Visitor for StaticInference {
    ...
    fn finish(
        ...
        if comp.name != "main" {
            match Self::get_possible_latency(&comp.control.borrow()) {
                Some(val) => {
                    let comp_sig = comp.signature.borrow();
                    let mut go_ports: Vec<_> =
                        comp_sig.find_all_with_attr(ir::NumAttr::Go).collect();
                    go_ports.iter_mut().for_each(|port| {
                        port.borrow_mut()
                            .attributes
                    ...
                    self.static_component_latencies
                        .insert(comp.name, NonZeroU64::new(val).unwrap());
                }
                None => (),
            }
        }
    ...
    fn start(
        ...
        for group in comp.get_groups() {
            let latency_result = if let Some(latency) =
                self.infer_latency(&group.borrow())
            {
    ...
    fn finish_seq(
        ...
        // sum = Some(latency) if we can infer latencies for all the children.
        // Otherwise, sum = None.
        let sum = s.stmts.iter().fold(Some(0), |acc, stmt| {
            match (acc, Self::get_possible_latency(stmt)) {
        ...
        // If sum is Some(), then we attach @promotable attribute.
        match sum {
            Some(latency) => {
                s.attributes.insert(ir::NumAttr::PromoteStatic, latency)
            }
    ...
    fn finish_par(
        ...
        // max = Some(latency) if we can infer latencies for all the children.
        // Otherwise, max = None.
        let max = s.stmts.iter().fold(Some(0), |acc, stmt| {
            match (acc, Self::get_possible_latency(stmt)) {
                (Some(cur_latency), Some(stmt_latency)) => {
    ...
    fn finish_if(
        ...
        match (
            Self::get_possible_latency(&s.tbranch),
            Self::get_possible_latency(&s.fbranch),
        ) {
            // If both branches have a known latency, then we attach the @promotable
            // attribute
            (Some(tbranch_latency), Some(fbranch_latency)) => {
                s.attributes.insert(
    ...
    fn finish_while(
        ...
        match (
            Self::get_possible_latency(&s.body),
            s.get_attributes().get(ir::NumAttr::Bound),
        ) {
            // If loop is bound and its body has a known latency, we can
            // attach the @promotable attribute
            (Some(body_latency), Some(num_repeats)) => s
                .attributes
                .insert(ir::NumAttr::PromoteStatic, body_latency * num_repeats),
            (_, _) => (),
        };
        Ok(Action::Continue)
    }
    ...
    fn finish_repeat(
        ...
        // If the body has a known latency, we can attach the @promotable attribute
        match Self::get_possible_latency(&s.body) {
            // If loop is bound and its body has a known latency, we can
            // attach the @promotable attribute
            Some(body_latency) => s
                .attributes
                .insert(ir::NumAttr::PromoteStatic, body_latency),
            None => (),
        };
        Ok(Action::Continue)
    }
