...
fn is_compatible(longer: &ir::Seq, shorter: &ir::Seq) -> Option<Vec<usize>> {
    ...
    while let (Some(c1), Some(c2)) = (long_val, short_val) {
        let x1 = c1.get_attribute(ir::NumAttr::Static)?;
        let x2 = c2.get_attribute(ir::NumAttr::Static)?;
        if x2 <= x1 {
...
fn attribute_with_static(v: u64) -> ir::Attributes {
    ...
    atts.insert(ir::NumAttr::Static, v);
    atts
}
...
fn get_static_sum(seq: &ir::Seq) -> Option<u64> {
    ...
    for stmt in &seq.stmts {
        count += stmt.get_attribute(ir::NumAttr::Static)?;
    }
    Some(count)
}
            ...
            let new_pars_static = new_pars_stmts
                ...
                .map(|stmts| {
                    stmts
                        .iter()
                        .map(|stmt| {
                            stmt.get_attribute(ir::NumAttr::Static).unwrap()
                        })
                        .max()
                })
